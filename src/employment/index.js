...
exports.handler = async function(event) {
        ...
        switch (event.httpMethod) {
            case "GET":
               if (event.pathParameters != null) {
                   body = await getEmploymentById(event);  // GET /employment/{id}
               } else {
                   body = await getEmploymentByApplicant(event); // GET /employment?appId
               }
               break;
               case "POST":
                   body = await createEmployment(event);  // POST /employment
                   break;
               case "DELETE":
                   body = await deleteEmployment(event); // DELETE /employment/{id}
                   break; 
               case "PUT":
                   body = await updateEmployment(event);  // PUT /employment/{id}
                   break;    
               default:
  ...
  const getEmploymentByApplicant = async (event) => {
    console.log(`getEmploymentByApplicant function. event: "${JSON.stringify(event)}"`);
  ...
  const getEmploymentById = async (event) => {
    console.log(`getEmploymentById function. event: "${JSON.stringify(event)}"`);

    try {
       const applicantId = event.queryStringParameters.appId;
       const employmentId = event.pathParameters.id;
       const params = {
           TableName: process.env.DYNAMODB_TABLE_NAME,
           Key: marshall({ PK: applicantId, SK: employmentId })
       }
...
*/
  ...
  const createEmployment = async (event) => {
    console.log(`createEmplyment function. event: "${JSON.stringify(event)}"`);
    ...
    try {
       ...
       // applicantId = removeSpaceToUpper(applicantId);
        ...
        const sortKey =  `EMPL:${nanoid(7).toString()}`;
  ...
  const deleteEmployment = async (event) => {
    console.log(`deleteEmployment function. event: "${JSON.stringify(event)}"`);
  ...
  const updateEmployment = async (event) => {
    console.log(`updateEmployment function. event: "${JSON.stringify(event)}"`);
